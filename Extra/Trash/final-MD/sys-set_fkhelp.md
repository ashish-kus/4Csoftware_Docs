...
### Arguments
alpha `<fkhelp>` - This can be "ON", "OFF", or "STATUS" Any other value is ignored and causes an error return.
...
### Example
In the system program `sys.help.set`, the EndFld PCL for ok calls `sethelp()` which in turn calls `sys.set_fkhelp()` to set the user specified value for XLFKHELPMSG. The entire `sethelp()` PCL follows:
```c
