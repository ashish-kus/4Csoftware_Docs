...
### Example
See the calls to `sys.link_prog` in the program development bootstraps. Especially note how LP_FROMANCESTOR is used in `sys.df.fm.pnl` and `sys.df.fm.def`. The LP_FROMANCESTOR in these two programs makes the `sys.link_prog` act like the link was made from `sys.df.maint1`.
