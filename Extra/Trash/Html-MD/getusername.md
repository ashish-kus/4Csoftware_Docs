...
sys.get_username() returns the user name of the user identified by
\<useridx\>.
...
integer \<useridx\> - The index into the 4C user info table. This must
be \>= 0 and \< sys.get_maxusers().
...
sys.get_username() can be called from anywhere. It is used by some of
the system configuration and display programs.
...
sys.get_username() returns the user name of the user identified by
\<useridx\>. The user info table is an internal table maintained by 4C.
Some entries may be blank due to users logging on and off. The main
purpose of this routine is to aid in system programs that display
