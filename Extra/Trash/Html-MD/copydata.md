...
sys.copy_data() copys all non-primary key fields from the data buffers
of one file to the data buffers of another.

Usage:

ret = sys.copy_data(\[ \<fromprog\>, \] \<fromfile\>, \[ \<toprog\>, \]
\<tofile\>);
...
The bootstrap program sys.prog.cpy uses sys.copy_data when copying the
control break information. The following is a copy of the PCL that
copies the sys.cb_hdr data.\
\
...
\
\
NOTE: The pcl name is copycbh() and it is the DrSelect PCL for the cbh1
driver.

Description:

sys.copy_data() copies all non-primary key fields from one files buffer
area to anothers. This does not do any kind of file access or update. It
is most useful when the same file is used more than once in the same
program. It should not be used UNLESS the two files have the EXACT same
file definition. If it is, fields that do not match in data type are not
copied.
