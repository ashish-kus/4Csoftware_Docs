...
The following shows how you can compare all data fields in two different
...
sys.cmp_data() compares each field in two different asfiles until all
fields of one file have compared ok or until there is a compare fail.
The difference between sys.cmp_data() and sys.cmp_file() is that
sys.cmp_data() does not compare primary key fields. It is not necessary
that the primary key layout of the two asfiles in the commpare be
compatible, but the data field layouts must be compatible.
