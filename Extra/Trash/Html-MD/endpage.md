...
sys.end_page() sets the next driver state of the program driver to be
ENDPAGE.
...
sys.end_page() can be called only while there is a a program driver
...
sys.end_page() is used to change the driver state of the program driver.
The next state is set to be ENDPAGE. All current executing PCLs will
continue executing until finished. The next action started by the
program driver will be to run the EPagePCL. It only makes sense to call
this PCL during the DrProcPCL or during the printing of the current
...
execute twice for the same record when sys.end_page() is called from
there.\
\
You may want to call sys.end_page() in order to get your program to
...
DRSELEOF state.\
\
When sys.dr_endpage() is called from the DRPROC PCL, this rcd will be
processed twice and the DRPROC PCL will be run twice on this rcd. It is
better to avoid calling sys.dr_endpage() from the DRPROC PCL. Call it
from one of the CB PCLS and no PCL will be executed twice for the same
rcd.

See Also:

[sys.exit_page()](./exitpage.html)
