...
"NOSELECT" - The return key will not select the rcd in select screens. Only the \<accept\> key will select the rcd.
...
### Example
In the system program sys.help.set, the Init PCL calls `sys.get_return()` in order to display the current value before allowing the user to change it. The entire Init PCL follows:
