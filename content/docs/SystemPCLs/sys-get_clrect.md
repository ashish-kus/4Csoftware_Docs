...
### Description
sys.get_clrect() returns the dimensions a single client control in system variables. The dimensions used are the same as those used by the client. For a win32 client, the dimensions are in pixels, while in a MacOS X Client, the dimensions are in points. The system variables that get the dimensions are `sys.rect_left`, `sys.rect_right`, `sys.rect_top`, and `sys.rect_bottom`. Allowable values for `<controlid>` are:
...
- "Panel" - The dimensions of the panel containing the 4C control that currently has the focus.
