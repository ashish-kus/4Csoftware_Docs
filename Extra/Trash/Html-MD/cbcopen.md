...
asfile - \<asfile\> - The 4C asfile that is used as a template for
formatting data.
...
sys.cbc_open() can be called anytime after sys.cbc_init() has been
called to begin the copy for the specified type.

Example:

Description:

sys.cbc_open() begins a specific type of clipboard copy operation. All
sys.cbc_open() calls need to have a corresponding sys.cbc_close() call.
Only one clipboard format can be open for copying at any one time.
